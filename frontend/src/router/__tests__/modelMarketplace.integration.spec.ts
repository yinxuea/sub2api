...
describe('model marketplace router integration', () => {
  ...
  it('uses the real router guard to redirect guest purchase navigation to login with redirect query', async () => {
    ...
    expect(router.currentRoute.value.query.redirect).toBe('/purchase?tab=subscription&plan_id=8')
    expect(router.currentRoute.value.fullPath).toBe('/login?redirect=/purchase?tab=subscription%26plan_id=8')
  ...
  it('completes the marketplace purchase redirect after a 2FA login on the real router', async () => {
    ...
    expect(router.currentRoute.value.query.tab).toBe('subscription')
    expect(router.currentRoute.value.query.plan_id).toBe('8')
